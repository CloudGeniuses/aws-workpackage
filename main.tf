...
resource "aws_eip" "management_nat" { domain = "vpc" tags = { Name = "management-nat-eip" } }
resource "aws_eip" "app_nat"        { domain = "vpc" tags = { Name = "app-nat-eip" } }
resource "aws_eip" "inspection_nat" { domain = "vpc" tags = { Name = "inspection-nat-eip" } }
...
resource "aws_route_table" "management_public" { vpc_id = aws_vpc.management.id tags = { Name = "management-public-rt" } }
...
resource "aws_route_table" "app_public" { vpc_id = aws_vpc.app.id tags = { Name = "app-public-rt" } }
...
resource "aws_route_table" "inspection_public" { vpc_id = aws_vpc.inspection.id tags = { Name = "inspection-public-rt" } }
...
resource "aws_route_table" "management_private" { vpc_id = aws_vpc.management.id tags = { Name = "management-private-rt" } }
...
resource "aws_route_table" "app_private" { vpc_id = aws_vpc.app.id tags = { Name = "app-private-rt" } }
...
resource "aws_route_table" "inspection_private" { vpc_id = aws_vpc.inspection.id tags = { Name = "inspection-private-rt" } }
...
# Associate Palo mgmt subnet to private RT (gets NAT + TGW routes)
...
########################################
# TRANSIT GATEWAY (keep defaults; no explicit re-assoc)
########################################

resource "aws_ec2_transit_gateway" "tgw" {
  description = "Central TGW for inter-VPC routing"
  # Keep defaults (auto-association enabled) to avoid AlreadyAssociated errors
  tags = { Name = "Main-TGW" }
}
...
resource "aws_ec2_transit_gateway_vpc_attachment" "management_attach" {
  ...
  vpc_id             = aws_vpc.management.id
  tags               = { Name = "TGW-Attach-Management" }
}
...
resource "aws_ec2_transit_gateway_route_table" "main" {
  transit_gateway_id = aws_ec2_transit_gateway.tgw.id
  tags               = { Name = "TGW-Main-Route-Table" }
}

# TGW routes to each VPC CIDR in the TGW RT
...
# VPC route tables -> TGW (explicit depends_on so TGW & attachments exist first)
resource "aws_route" "management_to_app_via_tgw" {
  route_table_id         = aws_route_table.management_private.id
  destination_cidr_block = var.app_vpc_cidr
  transit_gateway_id     = aws_ec2_transit_gateway.tgw.id
  depends_on             = [aws_ec2_transit_gateway.tgw, aws_ec2_transit_gateway_vpc_attachment.management_attach, aws_ec2_transit_gateway_vpc_attachment.app_attach]
}
...
resource "aws_route" "management_to_inspection_via_tgw" {
  ...
  destination_cidr_block = var.inspection_vpc_cidr
  transit_gateway_id     = aws_ec2_transit_gateway.tgw.id
  depends_on             = [aws_ec2_transit_gateway.tgw, aws_ec2_transit_gateway_vpc_attachment.management_attach, aws_ec2_transit_gateway_vpc_attachment.inspection_attach]
}
...
resource "aws_route" "app_to_management_via_tgw" {
  ...
  destination_cidr_block = var.management_vpc_cidr
  transit_gateway_id     = aws_ec2_transit_gateway.tgw.id
  depends_on             = [aws_ec2_transit_gateway.tgw, aws_ec2_transit_gateway_vpc_attachment.app_attach, aws_ec2_transit_gateway_vpc_attachment.management_attach]
}
...
resource "aws_route" "app_to_inspection_via_tgw" {
  ...
  destination_cidr_block = var.inspection_vpc_cidr
  transit_gateway_id     = aws_ec2_transit_gateway.tgw.id
  depends_on             = [aws_ec2_transit_gateway.tgw, aws_ec2_transit_gateway_vpc_attachment.app_attach, aws_ec2_transit_gateway_vpc_attachment.inspection_attach]
}
...
resource "aws_route" "inspection_to_management_via_tgw" {
  ...
  destination_cidr_block = var.management_vpc_cidr
  transit_gateway_id     = aws_ec2_transit_gateway.tgw.id
  depends_on             = [aws_ec2_transit_gateway.tgw, aws_ec2_transit_gateway_vpc_attachment.inspection_attach, aws_ec2_transit_gateway_vpc_attachment.management_attach]
}
...
resource "aws_route" "inspection_to_app_via_tgw" {
  ...
  destination_cidr_block = var.app_vpc_cidr
  transit_gateway_id     = aws_ec2_transit_gateway.tgw.id
  depends_on             = [aws_ec2_transit_gateway.tgw, aws_ec2_transit_gateway_vpc_attachment.inspection_attach, aws_ec2_transit_gateway_vpc_attachment.app_attach]
}
...
resource "aws_security_group" "app_sg" {
  ...
  ingress { from_port = 80  to_port = 80  protocol = "tcp" cidr_blocks = [aws_vpc.inspection.cidr_block] }
  ingress { from_port = 443 to_port = 443 protocol = "tcp" cidr_blocks = [aws_vpc.inspection.cidr_block] }
...
resource "aws_security_group" "palo_mgmt_sg" {
  ...
  # Broad intra-Inspection (optional)
  ingress { description = "HTTPS from Inspection VPC" from_port = 443 to_port = 443 protocol = "tcp" cidr_blocks = [var.inspection_vpc_cidr] }
  ingress { description = "SSH from Inspection VPC"   from_port = 22  to_port = 22  protocol = "tcp" cidr_blocks = [var.inspection_vpc_cidr] }

  # Narrow from bastion subnet (CIDR only; no cross-VPC SG refs)
  ingress { description = "HTTPS from mgmt-private (bastion)" from_port = 443 to_port = 443 protocol = "tcp" cidr_blocks = [local.mgmt_private_cidr] }
  ingress { description = "SSH from mgmt-private (bastion)"   from_port = 22  to_port = 22  protocol = "tcp" cidr_blocks = [local.mgmt_private_cidr] }

  egress  { description = "Allow all outbound" from_port = 0 to_port = 0 protocol = "-1" cidr_blocks = ["0.0.0.0/0"] }

  tags = { Name = "palo-mgmt-sg" }
}
